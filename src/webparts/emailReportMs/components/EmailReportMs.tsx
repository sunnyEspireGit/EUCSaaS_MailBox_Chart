...
import styles from './EmailReportMs.module.scss';
import { IEmailReportMsProps } from './IEmailReportMsProps';
...
export default class EmailReport extends React.Component<IEmailReportMsProps, IEmailReportStates,{}> {
  ...
  public render(): React.ReactElement<IEmailReportMsProps> {
    const {
      description,
      isDarkTheme,
      environmentMessage,
    ...
    return (
      <section className={`${styles.emailReportMs} ${hasTeamsContext ? styles.teams : ''}`}>
        ...
        <div>
          ...
          <div id='table' >
                    ...
                    {/* <table>
                      ...
                      <tr>
                        {this.state.finalArray_Google.map(column => <><td ><span style={{fontWeight:"bold"}}>{column.SubTags} :</span> {column.occurrence}</td></>)}
                      </tr>
