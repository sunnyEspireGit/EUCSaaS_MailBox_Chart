...
import { Dropdown as FabricDropdown, IDropdownOption} from "office-ui-fabric-react";
...
export default class ChartWp extends React.Component<IChartWpProps, IChartWpStates,{}> {
  ...
  public render(): React.ReactElement<IChartWpProps> {
    ...
    return (
      <section className={`${styles.chartWp} ${hasTeamsContext ? styles.teams : ''}`}>
        <div className={styles.welcome}>
          ...
          <div> 
            ...
            <ChartControl 
              type = {this.state.selected_chartType}
              // data={{
              ...
              loadingtemplate = {() => <Spinner size={SpinnerSize.large} label="Loading..."></Spinner>}
  ...
  private categoryChanged = (event: React.FormEvent<HTMLDivElement>, item: IDropdownOption): void => {
    ...
    console.log("this.state.selected_chartType - " , this.state.selected_chartType);
  ...
  const res_AllListData_Array = await sp.web.lists.getByTitle("ProductSupportEmailsReport").items.select("*").get();
